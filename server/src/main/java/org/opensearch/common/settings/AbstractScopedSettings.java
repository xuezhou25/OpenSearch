...
import org.opensearch.OpenSearchInvalidArgument;
...
public abstract class AbstractScopedSettings {
    ...
    protected AbstractScopedSettings(
        ...
        for (Setting<?> setting : settingsSet) {
            if (setting.getProperties().contains(scope) == false) {
                throw new OpenSearchInvalidArgument(
    ...
    protected void validateSettingKey(Setting<?> setting) {
        if (isValidKey(setting.getKey()) == false
            ...
            throw new OpenSearchInvalidArgument("illegal settings key: [" + setting.getKey() + "]");
        }
    }
    ...
    */
    ...
    public synchronized <T> void addSettingsUpdateConsumer(Setting<T> setting, Consumer<T> consumer, Consumer<T> validator) {
        if (setting != get(setting.getKey())) {
            throw new OpenSearchInvalidArgument("Setting is not registered for key [" + setting.getKey() + "]");
        }
    ...
    public synchronized <A, B> void addSettingsUpdateConsumer(
        ...
        if (a != get(a.getKey())) {
            throw new OpenSearchInvalidArgument("Setting is not registered for key [" + a.getKey() + "]");
        }
        if (b != get(b.getKey())) {
            throw new OpenSearchInvalidArgument("Setting is not registered for key [" + b.getKey() + "]");
        }
    ...
    void validate(
        ...
        if (setting == null) {
            ...
            throw new OpenSearchInvalidArgument(msg);
        } else {
            ...
            if (validateDependencies && settingsDependencies.isEmpty() == false) {
                for (final Setting.SettingDependency settingDependency : settingsDependencies) {
                    ...
                    if (dependency.existsOrFallbackExists(settings) == false) {
                        ...
                        throw new OpenSearchInvalidArgument(message);
                    }
    ...
    public <T> T get(Setting<T> setting) {
        ...
        if (get(setting.getKey()) == null) {
            throw new OpenSearchInvalidArgument("setting " + setting.getKey() + " has not been registered");
        }
    ...
    private boolean updateSettings(Settings toApply, Settings.Builder target, Settings.Builder updates, String type, boolean onlyDynamic) {
        ...
        for (String key : toApply.keySet()) {
            ...
            if (isDelete && (isValidDelete(key, onlyDynamic) || key.endsWith("*"))) {
                ...
                throw new OpenSearchInvalidArgument(type + " setting [" + key + "], not recognized");
                ...
                if (isFinalSetting(key)) {
                    throw new OpenSearchInvalidArgument("final " + type + " setting [" + key + "], not updateable");
                } else {
                    throw new OpenSearchInvalidArgument(type + " setting [" + key + "], not dynamically updateable");
                }
            }
        }
